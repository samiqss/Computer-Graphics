...
Sphere sphere[] = {
	...
	Sphere (vector3(1e6,250,250),1e6-400, vector3(130,140,150),DIFF),//right
	Sphere (vector3(-1e6,250,250),1e6+100, vector3(100,100,210),DIFF),//left
	Sphere (vector3(250,250,1e7),1e7, vector3(200,128,110),DIFF),//back
	Sphere (vector3(250,1e6,0),1e6-400, vector3(180,210,203),DIFF),//bottom
	Sphere (vector3(250,-1e6,0),1e6+100, vector3(210,210,210),DIFF),//top
	...
	Sphere (vector3(125*3+30,375,-6500),30, vector3(200, 160,80),AMB),//sphere3
	Sphere (vector3(125*2+30,200,-6500),30, vector3(200, 160,80),DIFF),//sphere4
	Sphere (vector3(125*2+30,0,-7500),1, vector3(250,250,250), DIFF),//lighot1 with yellow color
	Sphere (vector3(150,150,-7500),1, vector3(250,250,250), DIFF)//light2 with white color
	...
	for(int i=0;i<spheresNum; i++){//bright objects near by
		if(id != i && i < objectNum){
			...
			for(int j=0;j<objectNum; j++){//search for objects blocked from light
				double tt;
				Ray shadow= Ray(sphere[id].c,sphere[i].c-x);
				if(i != j && id != j && sphere[j].intersect(shadow,tt)){
				//	if(tt>t) temp.color = black;
				}
			}
			//
...
void RenderImage()
{
	...
	for( int x = 0; x < SCREENWIDTH; x++ )
	{
		for( int y = 0; y < SCREENHEIGHT; y++)
		{
			...
			const Ray cam(vector3(x,SCREENHEIGHT-y-1,-1e20),vector3(0,0,1));
