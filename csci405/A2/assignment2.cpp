...
Sphere sphere[] = {
	...
	Sphere (vector3(1e5,250,250),1e5-400, vector3(250,0,0),DIFF),//right
	Sphere (vector3(-1e5,250,250),1e5+100, vector3(100,100,210),DIFF),//left
	Sphere (vector3(250,250,1e6),1e6+700, vector3(130,160,90),DIFF),//back
	...
	Sphere (vector3(150,350,-6000),60, vector3(160,80,100),DIFF),//sphere1
	Sphere (vector3(125*2+30,375,-6500),70, vector3(100,160,80),SPEC),//sphere2
	Sphere (vector3(125*3+30,375,-9500),30, vector3(200, 160,80),MIR),//sphere3
	Sphere (vector3(300,0,-95000),1, vector3(250,250,0), DIFF),//lighot1 with yellow color
	Sphere (vector3(0,0,-25000),1, vector3(250,250,250), DIFF)//light2 with white color
};
...
vector3 reflect(const Ray r, int id, double t, int depth ){
	...
	for(int i=0;i<spheresNum; i++){//bright objects near by
		...
		if(id != i && i < objectNum){
			...
			if(temp.refl == MIR) temp.color = (temp.color+sphere[i].color*dt*0.8);//MIR
		        temp.color = temp.color + white*dt*0.25;//diffuse 
		
		}else if (i >= objectNum){//lights arounds

			vector3 L = sphere[i].c - x;//light
			vector3 N = sphere[id].getNormal(x);//normal
			double dt = vector3::DotProduct(L.Normalize(), N.Normalize());
			vector3 H = (L - r.d).Normalize(); //camera
			double dh = vector3::DotProduct(H.Normalize(), N.Normalize());

			if(temp.refl == SPEC || temp.refl == MIR) temp.color = (temp.color + white*0.4 + sphere[i].color*dt*0.6 + sphere[i].color*pow(dh,50)*0.6)*0.5;//spec
			else  temp.color = (temp.color + white*0.4 + sphere[i].color*dt*0.6)*0.5;//diff
			...
			for(int j=0;j<objectNum; j++){//search for objects blocked from light
				...
				double tobj = 1e14;
				double tlight = 1e14;
				Ray shadow= Ray(x+N*1e-3,L.Normalize());
				sphere[i].intersect(shadow,tlight);
				sphere[j].intersect(shadow,tobj);
				if(i != j && id != j){
					if(tobj<tlight) temp.color = black;
				}
			}
		}
	}
...
//loop over pixel, create image store it in buffer
void RenderImage()
{
	//background color
	const vector3 black(0, 0, 0);
	...
	for( int x = 0; x < SCREENWIDTH; x++ )
	{
		for( int y = 0; y < SCREENHEIGHT; y++)
		{
			...
			const Ray cam(vector3(x,SCREENHEIGHT-y-1,-1e4),vector3(0,0,1));
			...
			clamp255(pix_col);
			//insert colors to buffer
			gsPixelBuffer[bufferInd] = CRGB((int)pix_col.X(), (int)pix_col.Y(), (int)pix_col.Z());
		}
	}
}

//make all calls in main
